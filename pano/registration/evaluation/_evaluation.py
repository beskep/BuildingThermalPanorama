...
class BaseEvaluation:
  ...
  def plot(fi: RegisteringImage, mi: RegisteringImage, title: str = None):
    ...
    compare_orig_cb = tools.prep_compare_images(fi.resized_image(),
                                                mi.resized_image())
    compare_reg_cb = tools.prep_compare_images(fi.prep_image(),
                                               mi.registered_prep_image())
    compare_orig_diff = tools.prep_compare_images(fi.resized_image(),
                                                  mi.resized_image(),
                                                  method='diff')
    compare_reg_diff = tools.prep_compare_images(fi.prep_image(),
                                                 mi.registered_prep_image(),
                                                 method='diff')
  ...
  def execute_once(self, case, ff, mf, df, output_dir, **kwargs):
    ...
    mi = RegisteringImage(image=moving_image,
                          preprocess=self._moving_prep,
                          shape=fi.orig_image.shape)
  ...
  def execute(self, output_dir, fname, **kwargs):
    ...
    it = track(zip(self._cases, self._fixed_files, self._moving_files),
               total=len(self._cases))
    for case, ff, mf in it:
      try:
        self.execute_once(case=case,
                          ff=ff,
                          mf=mf,
                          df=df,
