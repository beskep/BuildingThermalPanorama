...
from misc.tools import ImageIO as IVIO
...
class TestImageIO:
  ...
  def save_image(self, directory):
    ...
    meta_path = path.with_name(
        f'{path.stem}{IVIO.META_SUFFIX}{tools.ImageIO.META_EXT}')
    assert meta_path.exists()

  def read_image(self, directory):
    path = Path(directory).joinpath(self.fname)

    img_npy, meta = IVIO.read_image_and_meta(path.with_suffix('.npy'),
                                             scale=False)
    ...
    img_csv, _ = IVIO.read_image_and_meta(path.with_suffix('.csv'), scale=False)
    assert self.arr == pytest.approx(img_csv)

    img_png, _ = IVIO.read_image_and_meta(path.with_suffix('.png'), scale=True)
